...
import { ArrowLeft, Crown, Check, Loader2, Pencil, Trophy } from "lucide-react";
import { C, NUM, SOMBRA } from "../tema";
import { Avatar, Boton, Rotulo } from "../components/ui";
import { fechaLarga, nombreCorto, votacionAbierta, cierreVotacion, mvpDePartido } from "../lib/util";
...
// Detalle de un partido con la votación del MVP. Cada asistente vota una vez
// identificándose con su PIN personal. La votación dura 24h.
export default function VotarMVP({ partido, jugadores, votos, onVotar, onCerrar, esAdmin, onEditar }) {
    ...
    <div className="fixed inset-0 z-[60] flex flex-col" style={{ background: C.fondo }}>
      <div className="flex items-center gap-3 px-4 py-3 shrink-0" style={{ background: C.tarjeta, boxShadow: SOMBRA }}>
        ...
        <div className="flex-1">
          <div className="font-extrabold" style={{ color: C.tinta }}>MVP del partido</div>
          <div className="text-xs capitalize" style={{ color: C.humo }}>{fechaLarga(partido.fecha)}</div>
        </div>
        {esAdmin && onEditar && (
          <button onClick={onEditar} className="p-2 active:opacity-60" title="Editar partido"><Pencil size={18} color={C.humo} /></button>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4" style={{ paddingBottom: "calc(24px + env(safe-area-inset-bottom))" }}>
        ...
        {abierta && yaVote && (
          <div className="mt-4 text-sm text-center font-semibold" style={{ color: C.primario }}>
            ¡Ya votaste! Gracias. Estos son los resultados en vivo.
          </div>
        )}
